...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onStart() {
        ...
        // Next step is the onConect event in the USBMonitor
    }
    ...
    private void calculateHistogram(ByteBuffer frame) {
        // Czyścimy koszyki
        int[] rBins = new int[256];
        int[] gBins = new int[256];
        int[] bBins = new int[256];

        // Frame to YUV (NV21). Interesuje nas głównie Y (jasność),
        // ale chcemy RGB. Uproszczona konwersja jest szybka.

        // Dostęp do bajtów
        int len = frame.capacity();
        // Bierzemy tylko próbki (np. co 20-ty piksel).
        // To daje ogromne przyspieszenie, a kształt wykresu jest identyczny.
        int step = 20;

        // Rozmiar obrazu Y (Luma) to width * height.
        // Kolor (UV) zaczyna się potem, ale dla histogramu RGB
        // w trybie podglądowym wystarczy nam sama analiza jasności (Y)
        // zabarwionej, albo pełna konwersja.

        // Zróbmy PEŁNĄ konwersję YUV->RGB na próbkach, żeby mieć kolory.
        // UVCCamera zwykle daje NV21.

        // Pobieramy wymiary (musimy je znać, np. z mRenderer.imageWidth)
        int width = 1920; // Załóżmy standard, lub pobierz z UVCCamera
        int height = 1080;
        int size = width * height;

        // Jeśli bufor jest mniejszy, nie ryzykujmy
        if (len < size) return;

        byte[] data = new byte[len];
        frame.get(data); // Kopiujemy dane (szybka operacja)

        for (int i = 0; i < size; i += step) {
            int Y = data[i] & 0xFF;

            // Uproszczenie: Histogram jasności (LUMA) jest najbezpieczniejszy i najszybszy.
            // Jeśli chcesz RGB z YUV, musisz sięgać do sekcji UV co jest skomplikowane w pętli.
            // Na start zróbmy Histogram RGB gdzie R=G=B=Y (Biały histogram ekspozycji).
            // Jeśli to zadziała, dorobimy kolory.
        ...
        runOnUiThread(() -> {
            if (histogramView != null) {
                histogramView.updateData(rBins, gBins, bBins);
            }
        });
    }
